...
# Contexto maximo real del modelo: 262144 tokens. Con --max-num-seqs 1 se
# aprovecha toda la ventana.
# Tool calling robusto via qwen3_coder. Sin --reasoning-parser porque el
# checkpoint nvidia no genera etiquetas <think></think>; con el parser activo
# el razonamiento plano queda en 'reasoning' y 'content' aparece vacio.
...
# Forzar thinking off: ENABLE_THINKING=false ./scripts/run-qwen36-35b-a3b.sh
...
# Ruta al chat-template relativa al repo (asumiendo ejecucion desde la raiz)
...
VLLM_ARGS=(
  ...
  --default-chat-template-kwargs "{\"enable_thinking\":${ENABLE_THINKING},\"preserve_thinking\":false,\"auto_disable_thinking_with_tools\":true}"
)
...
docker run -d --name "qwen36-vllm-a3b-${PORT}" \
  "${DOCKER_ARGS[@]}" \
  "${IMAGE}" \
  "${VLLM_ARGS[@]}"
