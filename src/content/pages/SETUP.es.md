...
- **Tool calling**: Qwen 3.6 requiere `--enable-auto-tool-choice --tool-call-parser qwen3_coder`; el parser `qwen3_coder` es más robusto para multi-turn que el anterior `qwen3_xml`. Sin parser, vLLM devuelve XML en `content` y el array nativo `tool_calls` queda vacío, por lo que agentes como Hermes/OpenClaw no ejecutan herramientas. Omitimos `--reasoning-parser` para el checkpoint `nvidia/Qwen3.6-35B-A3B-NVFP4` porque no genera etiquetas `<think></think>`; con el parser activo, el razonamiento queda en el campo `reasoning` y `content` aparece vacío en los agentes. Gemma 4 también tiene parser nativo `gemma4` por probar.
